...
SimulateRv32em::SimulateRv32em(Memory *memory) : Simulate(memory)
{
  reset();
}

SimulateRv32em::~SimulateRv32em()
{
}

Simulate *SimulateRv32em::init(Memory *memory)
{
  return new SimulateRv32em(memory);
}

void SimulateRv32em::reset()
{
  memset(reg, 0, sizeof(reg));
  pc = 0;
}

void SimulateRv32em::push(uint32_t value)
{
}

int SimulateRv32em::set_reg(const char *reg_string, uint32_t value)
{
...
uint32_t SimulateRv32em::get_reg(const char *reg_string)
{
  int index = get_register(reg_string);
...
void SimulateRv32em::set_pc(uint32_t value)
{
  pc = value;
}

void SimulateRv32em::dump_registers()
{
  char name[16];
  ...
  for (int n = 0; n < 32; n = n + 4)
  {
    ...
    {
      int index = n + i;
      snprintf(name, sizeof(name), "x%d/%s", index, rv32em_reg_names[index]);
...
int SimulateRv32em::run(int max_cycles, int step)
{
...
int SimulateRv32em::get_register(const char *name)
{
  if (strcasecmp(name, "s0") == 0) { return 8; }

  for (int n = 0; n < 32; n++)
  {
    if (strcasecmp(name, rv32em_reg_names[n]) == 0)
    {
      return n;
    }
  }
...
int SimulateRv32em::execute(uint32_t opcode)
{
...
int SimulateRv32em::jal_address(uint32_t opcode)
{
  int n = 0;
...
int SimulateRv32em::jalr_address(uint32_t opcode)
{
  int n = opcode >> 20;
...
int SimulateRv32em::branch(uint32_t opcode)
{
  int n = 0;
...
int SimulateRv32em::load(uint32_t opcode)
{
  const int rd = (opcode >> 7) & 0x1f;
  const int rs1 = (opcode >> 15) & 0x1f;
...
int SimulateRv32em::store(uint32_t opcode)
{
  const int rd = (opcode >> 7) & 0x1f;
  const int rs1 = (opcode >> 15) & 0x1f;
  const int funct3 = (opcode >> 12) & 0x7;
...
int SimulateRv32em::alu(uint32_t opcode)
{
  const int rd = (opcode >> 7) & 0x1f;
  const int rs1 = (opcode >> 15) & 0x1f;
...
int SimulateRv32em::alu_reg(uint32_t opcode)
{
  const int rd = (opcode >> 7) & 0x1f;
  const int rs1 = (opcode >> 15) & 0x1f;
